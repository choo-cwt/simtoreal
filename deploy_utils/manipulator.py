...
class LeRobotRealAgent(BaseRealAgent):
    ...
    def __init__(self, robot: Robot, use_cached_qpos: bool = True, **kwargs):
        ...
        self._joint_offsets_deg = self._load_joint_offsets()
        self._joint_signs = self._load_joint_signs()
        if self._joint_offsets_deg:
            print(f"Loaded real joint offsets for {self.real_robot.name}: {self._joint_offsets_deg}")
        if self._joint_signs:
            print(f"Loaded real joint signs for {self.real_robot.name}: {self._joint_signs}")

        if self.real_robot.name == "so100_follower":
            self.real_robot.bus.motors["gripper"].norm_mode = MotorNormMode.DEGREES
        elif self.real_robot.name in ("so101_follower", "so_follower"):
            self.real_robot.bus.motors["gripper"].norm_mode = MotorNormMode.DEGREES
    ...
    def _load_joint_offsets(self):
        path = Path(__file__).with_name("real_joint_offsets.json")
        if not path.exists():
            return {}
        data = json.loads(path.read_text())
        return data.get(self.real_robot.name, {})

    def _load_joint_signs(self):
        path = Path(__file__).with_name("real_joint_signs.json")
        if not path.exists():
            return {}
        data = json.loads(path.read_text())
        return data.get(self.real_robot.name, {})

    def _joint_transform_motors(self):
        return set(self._joint_offsets_deg) | set(self._joint_signs)

    def _sim_to_real_degrees(self, qpos_deg):
        for motor in self._joint_transform_motors():
            key = f"{motor}.pos"
            if key in qpos_deg:
                sign = float(self._joint_signs.get(motor, 1.0))
                offset = float(self._joint_offsets_deg.get(motor, 0.0))
                qpos_deg[key] = sign * qpos_deg[key] + offset

    def _real_to_sim_degrees(self, qpos_deg):
        for motor in self._joint_transform_motors():
            if motor in qpos_deg:
                sign = float(self._joint_signs.get(motor, 1.0))
                offset = float(self._joint_offsets_deg.get(motor, 0.0))
                qpos_deg[motor] = sign * (qpos_deg[motor] - offset)
    ...
    def set_target_qpos(self, qpos: Array):
        ...
        self._last_target_qpos = qpos.clone()
        ...
        if self.real_robot.name == "so100_follower":
            ...
            self._sim_to_real_degrees(qpos)
    ...
    def get_qpos(self):
        ...
        if self.real_robot.name == "so100_follower":
            ...
            self._real_to_sim_degrees(qpos_deg)
