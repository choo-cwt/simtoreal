...
#   "top" -> DefaultCameraEnv = TopCameraEnv
...
class RandomizationConfig:
    ...
    top_camera_pos_noise: Sequence[float] = (0.002, 0.002, 0.002)
    """Max position noise for a URDF-mounted top/head camera."""
    top_camera_rot_noise: Sequence[float] = (np.deg2rad(1), np.deg2rad(1), np.deg2rad(1))
    """Max rotation noise for a URDF-mounted top/head camera."""
    top_camera_fov_noise: float = np.deg2rad(1)
    """Noise scale for top/head camera FOV."""
...
class BaseRandomEnv(BaseEnv):
    ...
    def _default_human_render_camera_configs(self):
        pose = sapien_utils.look_at([1.1, -1.2, 0.8], [0.05, 0.0, 0.1])
    ...
    def _uniform_for_count(self, count, low, high, size=None):
        return self._uniform_for_indices(range(count), low, high, size=size)

    def _uniform_for_indices(self, indices, low, high, size=None):
        if torch.is_tensor(indices):
            indices = indices.detach().cpu().numpy()
        indices = np.asarray(list(indices), dtype=np.int64).reshape(-1)

        rngs = getattr(self._batched_episode_rng, "rngs", None)
        if rngs is None:
            values = np.asarray(self._batched_episode_rng.uniform(low, high, size=size))
            shape = (len(indices),) + (() if size is None else tuple(size))
            return np.resize(values, shape)
        if len(rngs) == 0:
            raise RuntimeError("Batched episode RNG has no per-env generators.")
        return np.asarray([
            rngs[int(i) % len(rngs)].uniform(low, high, size=size)
            for i in indices
        ])

    def _uniform_per_sub_scene(self, low, high, size=None):
        return self._uniform_for_count(len(self.scene.sub_scenes), low, high, size=size)

    def _fov_noise_per_env(self, scale):
        return scale * (2 * self._uniform_for_count(self.num_envs, 0.0, 1.0) - 1)
    ...
    def _load_lighting(self, options: dict):
        ...
        if self.domain_randomization and self.domain_randomization_config.randomize_lighting:
            ambient_colors = self._uniform_per_sub_scene(0.2, 0.5, size=(3,))
                ...
                for render_shape in render_body_component.render_shapes:
                    for part in render_shape.parts:
                        if (
                            ...
                            color = self._uniform_for_indices([i], 0.0, 1.0, size=(3,))[0].tolist()
                        else:
    ...
    def _randomize_gripper_speed(self, env_idx: torch.Tensor):
        ...
        env_ids = env_idx.detach().cpu().numpy().reshape(-1).astype(np.int64)
        stiffnesses = self._uniform_for_indices(env_ids, stiff_lo, stiff_hi)
        dampings = self._uniform_for_indices(env_ids, damp_lo, damp_hi)
        gripper_joint_name = getattr(self.agent, "gripper_joint_name", "gripper")
        gripper_joint = self.agent.robot.joints_map[gripper_joint_name]

        for i, idx in enumerate(env_ids.tolist()):
            stiffness = float(stiffnesses[i])
            damping = float(dampings[i])
            gripper_joint._objs[idx].set_drive_properties(stiffness, damping, force_limit=100)
            self._gripper_stiffness[idx] = stiffness
            self._gripper_damping[idx] = damping
    ...
    def _after_reconfigure(self, options: dict):
        ...
        if not self.domain_randomization_config.apply_overlay:
            self._objects_to_remove_from_greenscreen = []
            return
        if not hasattr(self, "_sensor_configs"):
            self._objects_to_remove_from_greenscreen = []
            return
    ...
    def _green_screen_rgb(self, rgb: torch.Tensor, segmentation: torch.Tensor, overlay: torch.Tensor) -> torch.Tensor:
        ...
        # Keep foreground objects (robot, task objects). Avoid torch.isin here:
        # with many vectorized environments it creates large temporary tensors.
        keep_ids = self._segmentation_ids_to_keep
        for keep_id in keep_ids.unbind():
            mask &= actor_seg != keep_id
        mask = mask[..., None]

        # Composite without promoting uint8 images to float. This avoids large
        # temporary allocations when many GPU environments render in parallel.
        return torch.where(mask, overlay, rgb)
...
class ThirdCameraEnv(BaseRandomEnv):
    ...
    def _default_sensor_configs(self):
        ...
        if self.domain_randomization and config.third_camera_fov_noise > 0:
            fov_noise = self._fov_noise_per_env(config.third_camera_fov_noise)
        else:
            fov_noise = 0
...
class WristCameraEnv(BaseRandomEnv):
    ...
    def _default_sensor_configs(self):
        ...
        if self.domain_randomization and config.wrist_camera_fov_noise > 0:
            fov_noise = self._fov_noise_per_env(config.wrist_camera_fov_noise)
        else:
            fov_noise = 0
...
class TopCameraEnv(BaseRandomEnv):
    ...
    def _default_sensor_configs(self):
        ...
        if self.domain_randomization and config.top_camera_fov_noise > 0:
            fov_noise = self._fov_noise_per_env(config.top_camera_fov_noise)
        else:
            fov_noise = 0
        ...
        return [
            CameraConfig(
                ...
                width=128,
                height=128,
                fov=self.TOP_CAMERA_FOV + fov_noise,
                near=0.01,
                far=100,
                mount=self.camera_mount,
            )
    ...
    def _update_top_camera_pose(self):
        if "head_camera_rgb_frame" not in self.agent.robot.links_map:
            raise RuntimeError("TopCameraEnv requires link 'head_camera_rgb_frame' in the robot URDF.")

        if self.gpu_sim_enabled:
            self.scene._gpu_fetch_all()

        head_pose = self.agent.robot.links_map["head_camera_rgb_frame"].pose
        local_pose = Pose.create_from_pq(
            p=torch.tensor(self.TOP_CAMERA_LOCAL_P, device=self.device).expand(self.num_envs, -1),
            q=torch.tensor(self.TOP_CAMERA_LOCAL_Q, device=self.device).expand(self.num_envs, -1),
        )
        self.camera_mount.set_pose(head_pose * local_pose)

        if self.gpu_sim_enabled:
            self.scene._gpu_apply_all()
            self.scene._gpu_fetch_all()

    def _hide_top_camera_self_visuals(self):
        for link_name in self.TOP_CAMERA_HIDE_SELF_LINKS:
            link = self.agent.robot.links_map.get(link_name)
            if link is None:
                continue
            for obj in link._objs:
                component = obj.entity.find_component_by_type(RenderBodyComponent)
                if component is not None:
                    component.visibility = 0

    def _get_obs_sensor_data(self, apply_texture_transforms: bool = True):
        self._hide_top_camera_self_visuals()
        self._update_top_camera_pose()
        return super()._get_obs_sensor_data(apply_texture_transforms)
...
if CAMERA_TYPE == "wrist":
    ...
    DefaultCameraEnv = TopCameraEnv
else:
    raise ValueError(f"Unknown CAMERA_TYPE: {CAMERA_TYPE}. Use 'wrist', 'third', or 'top'")
