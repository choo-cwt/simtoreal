...
class Lift(DefaultCameraEnv):
    ...
    **Success Conditions:**
    ...
    SUPPORTED_ROBOTS = ["so100", "so101", "xlerobot_right_head"]
    ...
    agent: Union[SO100, SO101, XLeRobot]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        elif robot_uids == "xlerobot_right_head":
            self.base_z_rot = np.pi / 2
    ...
    def _load_agent(self, options: dict):
        ...
        super()._load_agent(
            options,
            sapien.Pose(p=self.robot_base_pos, q=euler2quat(0, 0, self.base_z_rot)),
            build_separate=True
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            xyz = torch.zeros((b, 3))
            spawn_box_half_size = torch.as_tensor(self.spawn_box_half_size, device=self.device, dtype=torch.float32)
            if spawn_box_half_size.numel() == 1:
                spawn_box_half_size = spawn_box_half_size.expand(2)
            xyz[:, :2] = torch.rand((b, 2), device=self.device) * spawn_box_half_size * 2 - spawn_box_half_size
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        # Reach the item from above first. A raw 3D distance makes the easiest
        # first improvement "move downward", even when the gripper is not
        # horizontally aligned with the item yet.
        tcp_pos = self.agent.tcp_pose.p
        item_pos = self.item.pose.p
        xy_dist = torch.linalg.norm(item_pos[..., :2] - tcp_pos[..., :2], axis=1)
        pregrasp_z = item_pos[..., 2] + 0.08
        z_pregrasp_dist = torch.abs(tcp_pos[..., 2] - pregrasp_z)
        z_grasp_dist = torch.abs(tcp_pos[..., 2] - item_pos[..., 2])

        xy_reward = 1 - torch.tanh(10 * xy_dist)
        pregrasp_z_reward = 1 - torch.tanh(8 * z_pregrasp_dist)
        grasp_z_reward = 1 - torch.tanh(12 * z_grasp_dist)
        xy_near = (xy_dist < 0.08).float()
        xy_close = (xy_dist < 0.04).float()
        reaching_reward = xy_reward + xy_near * 0.5 * pregrasp_z_reward + xy_close * grasp_z_reward

        # After grasping, switch the objective from "stay at the item" to
        # "lift and return to rest"; otherwise the policy can settle for
        # holding the item at the pickup pose.
        grasped = info["is_item_grasped"].float()
        item_lift = torch.clamp(item_pos[..., 2] - self.item_half_sizes, min=0)
        lift_reward = 1 - torch.tanh(20 * torch.clamp(0.06 - item_lift, min=0))

        reward = reaching_reward * (1 - grasped)
        reward += grasped * (2.0 + 2.0 * lift_reward)

        # Lift back reward
        place_reward = torch.exp(-2 * info["distance_to_rest_qpos"])
        reward += 3.0 * place_reward * grasped
