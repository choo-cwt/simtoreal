    ...
    **Success Conditions:**
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.privileged_state = privileged_state
    ...
    def _uniform_per_env(self, low, high, size=None):
        rngs = getattr(self._batched_episode_rng, "rngs", None)
        if rngs is None:
            values = np.asarray(self._batched_episode_rng.uniform(low=low, high=high, size=size))
            return np.resize(values, (self.num_envs,) + (() if size is None else tuple(size)))
        if len(rngs) == 0:
            raise RuntimeError("Batched episode RNG has no per-env generators.")
        return np.asarray([
            rngs[i % len(rngs)].uniform(low=low, high=high, size=size)
    ...
    def _load_scene(self, options: dict):
        ...
        if self.item_type == "cube":
            ...
            if self.domain_randomization:
                half_radii = self._uniform_per_env(
                    low=cfg.can_radius_range[0],
                    high=cfg.can_radius_range[1],
                )
                half_heights = self._uniform_per_env(
        ...
        bin_color = sapien.render.RenderMaterial(base_color=[0.0, 0.8, 0.1, 1.0])
        thickness = 0.005
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            if isinstance(self.spawn_box_half_size, Sequence):
                spawn_box_half_size_x, spawn_box_half_size_y = self.spawn_box_half_size
            else:
                spawn_box_half_size_x = spawn_box_half_size_y = self.spawn_box_half_size
            region = [
                [-spawn_box_half_size_x, -spawn_box_half_size_y],
                [spawn_box_half_size_x, spawn_box_half_size_y]
            ]
