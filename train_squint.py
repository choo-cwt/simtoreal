...
class Args:
    ...
    """robot uid to load in the task. Use 'xlerobot_right_head' for the URDF in ./urdf."""
    no_privileged_state: bool = False
    """For place tasks, hide object/bin privileged state from the policy and keep only robot state."""
...
if __name__ == "__main__":
    ...
    best_model_path = os.path.abspath(f"runs/{run_name}/best_ckpt.pt")
    best_metrics_path = os.path.abspath(f"runs/{run_name}/best_metrics.txt")
    ...
    if args.no_privileged_state:
        ...
        eval_env_kwargs["privileged_state"] = False
    ...
    for iteration in range(args.num_total_iterations + 2):  # +2 for final eval
        ...
        if args.eval_freq > 0 and ((global_step - args.num_envs) // args.eval_freq) < (global_step // args.eval_freq):
            ...
            if args.save_model:
                ...
                success_at_end = float(eval_d["eval/success_at_end"])
                if success_at_end > best_success_at_end:
                    ...
                    with open(best_metrics_path, "w", encoding="utf-8") as f:
                        ...
                        for k in sorted(eval_d):
                            v = eval_d[k]
                            if torch.is_tensor(v):
                                v = v.detach().cpu().item()
                            f.write(f"{k}={v}\n")
                    print(
                        f"Step {global_step}: new best success_at_end={success_at_end:.4f}; "
                        f"best checkpoint saved to {best_model_path}"
                    )
        ...
        actions = torch.as_tensor(actions, device=device, dtype=torch.float).reshape(args.num_envs if not args.evaluate else 1, -1)
